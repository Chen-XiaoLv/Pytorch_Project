...
batch_size = 36  # 批次大小
input_dim = 1  # 每个步长对应的特征数量，这里只使用1维，每天电力消耗量
hidden_dim = 64  # 隐层大小
output_dim = 12  # 由于是多输出任务，最终输出层大小为7，预测未来7天电力消耗
num_layers = 3  # gru的层数
epochs = 10
best_loss = 0
model_name = 'lstm'
...
scaler.fit_transform(np.array(df['Val']).reshape(-1, 1))


# 形成训练数据，例如12345789 12-3456789
def split_data(data, timestep, input_dim, output_dim):
    dataX = []  # 保存X
    dataY = []  # 保存Y

    # 将整个窗口的数据保存到X中，将未来一天保存到Y中
    for index in range(len(data) - timestep - output_dim):
        dataX.append(data[index: index + timestep])
        dataY.append(data[index + timestep: index + timestep + output_dim])
...
x_train, y_train, x_test, y_test = split_data(data, timestep, input_dim, output_dim)
print(x_train.shape)
print(y_test.shape)
...
class GRU(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim):
        ...
        # input_dim为特征维度，就是每个时间点对应的特征数量，这里为1
...
model = GRU(input_dim, hidden_dim, num_layers, output_dim)  # 定义gru网络
...
for epoch in range(epochs):
    ...
    for data in train_bar:
        ...
        loss = loss_function(y_train_pred, y_train.reshape(-1, 12))
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for data in test_bar:
            x_test, y_test = data
            y_test_pred = model(y_test)
            test_loss = loss_function(y_test_pred, y_test.reshape(-1, 12))
