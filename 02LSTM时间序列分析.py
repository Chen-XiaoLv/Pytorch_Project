...
# 1.加载时间序列数据
df = pd.read_excel(r"C:\Users\lenovo\Desktop\pc1.xlsx")

# 2.将数据进行标准化
scaler = StandardScaler()
scaler_model = StandardScaler()
data = scaler_model.fit_transform(np.array(df))
scaler.fit_transform(np.array(df['pc2']).reshape(-1, 1))
...
plt.plot(scaler.inverse_transform((model(x_train_tensor[:10000], x_train_tensor[:10000], x_train_tensor[:10000]).detach().numpy()).reshape(-1, 1)), "b")
plt.plot(scaler.inverse_transform(y_train_tensor.detach().numpy().reshape(-1, 1)), "r")
plt.legend()
plt.show()

y_test_pred = model(x_test_tensor, x_test_tensor, x_test_tensor)
plt.figure(figsize=(12, 8))
